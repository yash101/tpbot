 ...
 * loglistener.cpp
 * 
 * A simple multicast log listener utility.
 * Listens on specified multicast groups and ports, prints received messages
 * 
 * The ESP32 firmware can be configured to log to multicast groups.
 * 
 * Note: this code is mostly vibe coded garbage. Prob don't trust it for production use.
 */
  ...
  std::array<uint8_t, 6> &out_mac)
{
  ...
  for (int i = 0; i < MAX_TRIES; ++i)
  {
    ...
    if (r < (ssize_t)(sizeof(struct ethhdr) + sizeof(struct iphdr)))
      continue;
    ...
    if (r < (ssize_t)(sizeof(struct ethhdr) + ihl + sizeof(struct udphdr)))
      continue;
...
int main(int argc, char **argv)
{
  ...
  for (const auto &g : groups)
  {
    ...
    {
      int on = 1;
      setsockopt(sock, IPPROTO_IP, IP_RECVDSTADDR, &on, sizeof(on));
    }
#endif
  ...
  while (keepRunning)
  {
    ...
    size_t printed_len = (n >= 0 && n < sizeof(iov_recvbuf)) ? n : (sizeof(iov_recvbuf) - 1);
    ...
    oss << std::put_time(std::localtime(&t), "%F %T")
        << " [d=" << (have_dst ? dst_ip_str : "?") << ":" << dst_port << ""
        << " s=" << src_ip_str << ":" << src_port << ""
        << " m=" << src_mac_str << "] "
        << 'msg=' << iov_recvbuf;
